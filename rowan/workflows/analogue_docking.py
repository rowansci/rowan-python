...
class AnalogueDockingResult(WorkflowResult):
    ...
    def get_pose(self, smiles: str, index: int = 0) -> Calculation:
        ...
        Args:
            smiles: SMILES string of the analogue
...
def submit_analogue_docking_workflow(
    ...
    Args:
        analogues: SMILES strings to dock
        initial_molecule: template to which to align molecules to
        protein: protein to dock. Can be input as a uuid or a Protein object
        analogue_names: names parallel to `analogues`. When provided, each docked
        ...
        scoring_function: docking scoring function: "vina" or "vinardo"
        exhaustiveness: how many times Vina attempts to find a pose for each conformer
        max_poses: maximum number of poses generated per input conformer
        num_conformers_per_analogue: maximum number of conformers to generate per analogue
        run_local_optimization: whether to run a local opt in docking pocket or just score
    ...
    with api_client() as client:
        ...
        return Workflow[AnalogueDockingResult](**response.json())
