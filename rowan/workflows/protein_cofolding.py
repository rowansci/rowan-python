...
class CofoldingScores:
    ...
    Attributes:
        ptm: predicted TM-score, overall structure confidence (0-1, higher is better)
        iptm: interface pTM, inter-chain packing confidence (0-1, higher is better)
        avg_lddt: mean per-residue pLDDT, local atomic accuracy (0-1, higher is better)
        confidence_score: overall aggregate confidence in the prediction (0-1, higher is better)
    """
...
class AffinityScore:
    ...
    Attributes:
        pred_value: predicted pIC50, -log10(IC50 in M); higher means stronger
            binding (ensemble average of the two affinity heads)
        pred_value1: predicted pIC50 from affinity head 1
        pred_value2: predicted pIC50 from affinity head 2
        probability_binary: predicted probability (0-1) that the ligand binds
            its target; higher is better (ensemble average of the two affinity heads)
        probability_binary1: binding probability (0-1) from affinity head 1
        probability_binary2: binding probability (0-1) from affinity head 2
        binding_confidence: predicted probability (0-1, higher is better) that
        ...
        optimization_score: binding-strength ranking derived from the model's
...
class CofoldingResult:
    ...
    Attributes:
        scores: confidence scores for the prediction
        affinity_score: predicted binding affinity (if computed)
        strain: ligand strain energy (if computed)
        mmgbsa_score: MM/GBSA binding free energy estimate in kcal/mol (if computed)
        posebusters_valid: whether the pose passes PoseBusters validation
        lddt: per-residue LDDT confidence scores
        pose_uuid: UUID of the pose
        predicted_structure_uuid: UUID of the predicted structure
        predicted_refined_structure_uuid: UUID of the refined structure (if refinement was run)
    """
...
def submit_protein_cofolding_workflow(
    ...
    Args:
        initial_protein_sequences: protein sequences to be cofolded
        initial_dna_sequences: DNA sequences to be cofolded
        initial_rna_sequences: RNA sequences to be cofolded
        initial_smiles_list: list of SMILES strings for the ligands to be cofolded with
        ligand_binding_affinity_index: index of the ligand for which to compute
            the binding affinity
        use_msa_server: whether to use the MSA server for the computation
        use_potentials: whether to use potentials (inference-time steering) with Boltz
        contact_constraints: Boltz contact constraints between two tokens
        pocket_constraints: Boltz pocket constraints between a binder and contact tokens
        bond_constraints: Boltz-2 covalent bond constraints between two atoms. Incompatible
            with pose refinement and strain calculation
        templates: structural templates to guide prediction (Boltz-2/2.1 or OpenFold-3 only)
        num_samples: number of diffusion samples to generate. If None, uses the model default
        compute_strain: whether to compute the strain of the pose. Requires do_pose_refinement.
            (if `pose_refinement` is enabled)
        do_pose_refinement: whether to optimize non-rotatable bonds in output poses
        name: name of the workflow
        model: model to use for the computation. Boltz-2.1 runs via Boltz's
    ...
    Raises:
        ValueError: no protein, DNA, or RNA sequences are provided
