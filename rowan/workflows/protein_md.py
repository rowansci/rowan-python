...
class ProteinMDTrajectory:
    ...
    Attributes:
        ...
        binder_rmsd: per-frame binder RMSD, when the binder has one component
        mmgbsa_scores: per-frame MM/GBSA score for the complete binder
...
def submit_protein_md_workflow(
    ...
    Args:
        ...
        small_molecule_ff: force field for small molecules
        protein_ff: force field for proteins
        water_ff: force field for water
        equilibration_time_ns: how long to equilibrate trajectories for, in ns
        simulation_time_ns: how long to run trajectories for, in ns
        temperature: temperature, in K
        pressure_atm: pressure, in atm
        langevin_timescale_ps: timescale for the Langevin integrator, in ps^-1
        timestep_fs: timestep, in femtoseconds
        hydrogen_mass: hydrogen mass, in atomic mass units
        constrain_hydrogens: whether or not to use SHAKE to freeze bonds to hydrogen
        ...
        small_molecules: SMILES by protein residue name or index for small molecules that
            require separate parameterization. A None value uses an existing residue template
        binder: optional binder specification (protein chains and/or small molecules).
        ...
        protein_restraint_cutoff: cutoff distance from the binder past which Cα atoms are
            harmonically restrained, in Å; None disables restraints
        protein_restraint_constant: force constant for Cα backbone restraints, in kcal/mol/Å²
