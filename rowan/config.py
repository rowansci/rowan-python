...
class MethodSettings:
    ...
    atoms_supported: list[int] | None = None
    disabled_tasks: list[Task] = field(default_factory=list)
    disable_basis_sets: bool = False
    disable_corrections: bool = False
    disable_optimize_cell: bool = False


_TO_RN = list(range(1, 87))  # H to Rn (1-86)
_TO_LR = list(range(1, 104))  # H to Lr (1-103)
...
class EngineSettings:
    ...
    methods: list[str]
    atoms_supported: list[int] | None = None
    disabled_tasks: Callable[[bool], list[Task]] | None = None
    disable_basis_sets: bool = False
    disable_corrections: bool = False
    corrections: list[str] = field(default_factory=list)
    disable_solvents: bool = False
    solvent_models: list[str] = field(default_factory=list)
    disable_open_shell: bool = False
    disable_charge: bool = False
    periodic_support: bool = False
    periodic_only: bool | str = False  # True, False, or "warn"


_MP_TRJ = [i for i in range(1, 96) if i not in [84, 85, 86, 87, 88]]
_OMOL25 = list(range(1, 84))  # H to Bi (1-83)
...
_TO_AR = list(range(1, 19))  # H to Ar (1-18)
_TO_KR = list(range(1, 37))  # H to Kr (1-36)
_TO_XE = list(range(1, 55))  # H to Xe (1-54)
_TO_KR_MINUS_K = [i for i in _TO_KR if i != 19]
_TO_KR_MINUS_K_CA = [i for i in _TO_KR if i not in [19, 20]]
_SC_TO_ZN = list(range(21, 31))
_TO_KR_MINUS_SC_TO_ZN = [i for i in _TO_KR if i not in _SC_TO_ZN]

BASIS_SET_ATOMS: dict[str, list[int]] = {
    "STO-3G": _TO_XE,
    "STO-6G": _TO_XE,
...
class ConformerGeneratorSettings:
    ...
    :ivar disable_constraints: Geometry constraints not supported.
    :ivar disable_open_shell: Only closed-shell molecules supported.
    :ivar disable_ts: Transition state search not supported.
    :ivar atoms_supported: Supported atomic numbers (None = all).
    :ivar allowed_engines: Engines allowed for energy evaluation (None = all).
    :ivar solvent_warning: Warn if generator solvent differs from final opt solvent.
    """
...
class MDSettings:
    ...
    :cvar TIMESTEP_MIN: Minimum timestep in femtoseconds.
    :cvar TIMESTEP_MAX: Maximum timestep in femtoseconds.
    :cvar MAX_SAVED_FRAMES: Maximum number of frames that can be saved.
    :cvar NPT_REQUIRES_PERIODIC: NPT ensemble only valid for periodic systems.
    :cvar QUASICLASSICAL_REQUIRES_FREQUENCIES: Quasiclassical init needs frequencies.
    :cvar READ_REQUIRES_VELOCITIES: Read init needs velocities from previous job.
    :cvar CONFINING_BLOCKED_FOR_PERIODIC: Confining constraints invalid for periodic.
    """
...
class SolubilityMethodSettings:
    ...
    :ivar allowed_solvents: Solvents supported (None = all). Use SMILES format.
    :ivar single_temperature_k: If set, only this temperature allowed.
    :ivar temperature_range: If set, (min_k, max_k) range allowed.
    """
...
class MacroPKaSettings:
    ...
    :cvar PH_WARNING_MIN: pH values below this trigger accuracy warning.
    :cvar PH_WARNING_MAX: pH values above this trigger accuracy warning.
    :cvar CHARGE_MIN: Minimum allowed charge state.
    :cvar CHARGE_MAX: Maximum allowed charge state.
    """
...
class CofoldingModelSettings:
    ...
    :ivar max_sequence_length: Maximum total sequence length.
    :ivar supports_cyclic_peptides: Whether cyclic peptides are supported.
    :ivar supports_potentials: Whether potentials can be used.
    :ivar supports_ligand_binding_affinity: Whether binding affinity available.
    :ivar ligand_max_atoms: Max atoms per ligand for binding affinity.
    :ivar ligand_atoms_supported: Supported atoms for ligand binding.
    """
    ...
    ligand_atoms_supported: list[int] | None = None
...
class BinderDesignSettings:
    ...
    :cvar MAX_SEQUENCE_LENGTH: Maximum total amino acids.
    :cvar NUM_DESIGNS_MAX_FREE: Max designs for free tier.
    :cvar NUM_DESIGNS_MAX_SUBSCRIBED: Max designs for subscribed users.
    :cvar BUDGET_MUST_BE_LEQ_NUM_DESIGNS: budget cannot exceed num_designs.
    :cvar REQUIRES_DESIGNABLE_REGIONS: At least one sequence must have digits.
    :cvar REQUIRES_UNIQUE_CHAIN_IDS: All chain IDs must be unique.
    """
...
def get_combined_settings(
    ...
    engine_disabled = (
        engine_settings.disabled_tasks(periodic) if engine_settings.disabled_tasks else []
    )
    method_disabled = method_settings.disabled_tasks or []
    combined_disabled = list(set(engine_disabled + method_disabled))

    atoms: list[int] | None
    if engine_settings.atoms_supported and method_settings.atoms_supported:
        atoms = list(set(engine_settings.atoms_supported) & set(method_settings.atoms_supported))
    else:
...
def get_conformer_generator_settings(generator: str) -> ConformerGeneratorSettings | None:
    """Get settings for a conformer generator, or None if not found."""
