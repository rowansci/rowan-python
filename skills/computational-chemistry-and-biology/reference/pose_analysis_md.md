...
- `trajectories`: per-replicate results. Each exposes `ligand_rmsd`, `protein_rmsd` (per-frame Cα RMSD from frame 0, Å), `rmsf` (per-Cα RMSF from the mean structure, Å), `potential_energy` (per-frame whole-system potential energy, Hartree), `mmgbsa_scores` (per-frame MM/GBSA interaction energy, kcal/mol), persistent protein-ligand `contacts`, and `isotropic_radius_of_gyration`. When `analysis_interval_ps` is set, `sasa` and `polar_sasa` are populated; when `clustering` is set, `cluster_centroid_indices` and `cluster_indices_by_frame` are populated.
